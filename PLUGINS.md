...
### Run pc in dev mode
pc can be run in dev mode by cloning this repository and following the ["Contributing" section of our README](https://github.com/monadicus/pc#contribute).

In dev mode you'll get more ouput and access to React/Redux dev-tools in Electron.

Prerequisites and steps are described in the ["Contributing" section of our README](https://github.com/monadicus/pc#contribute).
Be sure to use the `canary` branch.

### Create a dev config file
Copy your config file `.pc.js` to the root of your cloned repository. pc, in dev mode, will use this copied config file. That means that you can continue to use your main installation of pc with your day-to-day configuration.
After the first run, pc, in dev mode, will have created a new `.pc_plugins` directory in your repository directory.

### Setup your plugin
Go to your recently created `<repository_root>/.pc_plugins/local` directory and create/clone your plugin repo. An even better method on macOS/Linux is to add a symlink to your plugin directory.
...
module.exports = {
  ...
  plugins: [],
  localPlugins: ['pc-awesome-plugin'],
  ...
}
```

### Running your plugin
To load, your plugin should expose at least one API method. All possible methods are listed [here](https://github.com/monadicus/pc/blob/canary/app/plugins/extensions.js).

After launching pc in dev mode, run `yarn run app`, it should log that your plugin has been correcty loaded: `Plugin pc-awesome-plugin (0.1.0) loaded.`. Name and version printed are the ones in your plugins `package.json` file.
...
### Components
You can decorate almost all pc components with a Higher-Order Component (HOC). To understand their architecture, the easiest way is to use React dev-tools to dig in to their hierachy.

Multiple plugins can decorate the same pc component. Thus, `Component` passed as first argument to your decorator function could possibly not be an original pc component but a HOC of a previous plugin. If you need to retrieve a reference to a real pc component, you can pass down a `onDecorated` handler.
```js
...
**Bonus feature**: if your command ends with `:prefix`, it would mean that you want to use this command with an additional digit to the command. Then pc will create all your commands under the hood. For example, this keymap `'pane:hide:prefix': 'ctrl+shift'` will automatically generate the following:
```
{
...
### Menu
Your plugin can expose a `decorateMenu` function to modify the pc menu template.
...
## pc v2 breaking changes
pc v2 uses `xterm.js` instead of `hterm`. It means that PTY ouput renders now in a canvas element, not with a hackable DOM structure.
