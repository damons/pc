    ...
    return (
      <div id="pc">
        <div
          ...
          className={`pc_main ${isMac_ && 'pc_mainRounded'}`}
        >
          <HeaderContainer />
        ...
        {/*
          Add custom CSS to pc.
          We add a scope to the customCSS so that it can get around the weighting applied by styled-jsx
        */}
        <style dangerouslySetInnerHTML={{__html: stylis('#pc', customCSS, {prefix: false})}} />
      </div>
    );
  }
}

const PcContainer = connect(
  state => {
    return {
      isMac,
