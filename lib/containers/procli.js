...
class Procli extends React.PureComponent {
  constructor(props) {
    super(props);
    ...
    return (
      <div id="procli">
        <div
          ...
          className={`procli_main ${isMac_ && 'procli_mainRounded'}`}
        >
          <HeaderContainer />
        ...
        {/*
          Add custom CSS to Procli.
          We add a scope to the customCSS so that it can get around the weighting applied by styled-jsx
        */}
        <style dangerouslySetInnerHTML={{__html: stylis('#procli', customCSS, {prefix: false})}} />
      </div>
    );
  }
}

const ProcliContainer = connect(
  state => {
    return {
      isMac,
...
export default ProcliContainer;
