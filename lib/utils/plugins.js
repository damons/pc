...
// https://github.com/monadicus/procli/issues/619
...
Module._load = function _load(path) {
  switch (path) {
    ...
    case 'react-dom':
      return ReactDOM;
    case 'procli/component':
      return PureComponent;
    case 'procli/notify':
      return notify;
    case 'procli/Notification':
      return Notification;
    case 'procli/decorate':
      return decorate;
    default:
...
const loadModules = () => {
  ...
  modules = paths.plugins
    ...
    .map(path => {
      ...
      // mapProcliTermState mapping for backwards compatibility with procliterm
      if (mod.mapProcliTermState) {
        mod.mapProcliState = mod.mapProcliTermState;
        //eslint-disable-next-line no-console
        console.error('mapProcliTermState is deprecated. Use mapProcliState instead.');
      }

      // mapProcliTermDispatch mapping for backwards compatibility with procliterm
      if (mod.mapProcliTermDispatch) {
        mod.mapProcliDispatch = mod.mapProcliTermDispatch;
        //eslint-disable-next-line no-console
        console.error('mapProcliTermDispatch is deprecated. Use mapProcliDispatch instead.');
      }
      ...
      if (mod.mapProcliState) {
        connectors.Procli.state.push(mod.mapProcliState);
      }

      if (mod.mapProcliDispatch) {
        connectors.Procli.dispatch.push(mod.mapProcliDispatch);
      }
