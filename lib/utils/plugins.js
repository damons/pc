...
// https://github.com/monadicus/pc/issues/619
...
const loadModules = () => {
  ...
  modules = paths.plugins
    ...
    .map(path => {
      ...
      // mappcTermState mapping for backwards compatibility with pcterm
      if (mod.mappcTermState) {
        mod.mappcState = mod.mappcTermState;
        //eslint-disable-next-line no-console
        console.error('mappcTermState is deprecated. Use mappcState instead.');
      }

      // mappcTermDispatch mapping for backwards compatibility with pcterm
      if (mod.mappcTermDispatch) {
        mod.mappcDispatch = mod.mappcTermDispatch;
        //eslint-disable-next-line no-console
        console.error('mappcTermDispatch is deprecated. Use mappcDispatch instead.');
      }
      ...
      if (mod.mappcState) {
        connectors.pc.state.push(mod.mappcState);
      }

      if (mod.mappcDispatch) {
        connectors.pc.dispatch.push(mod.mappcDispatch);
      }
