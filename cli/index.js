...
const checkConfig = () => {
  ...
  msg += 'Please launch pc and retry.';
  console.error(msg);
  process.exit(1);
};
...
const lsRemote = pattern => {
  ...
  const URL = `https://api.npms.io/v2/search?q=${(pattern && `${pattern}+`) || ''}keywords:pc-plugin,pc-theme`;
  return got(URL)
  ...
  commandPromise = lsRemote(query)
    .then(entries => {
      if (entries.length === 0) {
        ...
        console.error(`${chalk.red('Try')} ${chalk.green('pc ls-remote')}`);
        process.exit(1);
      } else {
...
args.command(['<default>'], 'Launch pc');
...
const main = argv => {
  ...
  const env = Object.assign({}, process.env, {
    // this will signal pc that it was spawned from this module
    HYPER_CLI: '1',
  ...
  if (!flags.verbose) {
    ...
    if (process.platform === 'darwin') {
      //Use `open` to prevent multiple pc process
      const cmd = `open -b co.monadicus.pc ${args_}`;
      const opts = {
        env
      };
